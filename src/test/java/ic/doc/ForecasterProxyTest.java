...
public class ForecasterProxyTest {
  ...
  public void queryInLowerCaseWillBeConvertedToUpperCase () {
    ...
    context.checking(new Expectations() {{
      exactly(1).of(cache).getFromCache(keys); will(returnValue(null));
      exactly(1).of(forecaster).forecastFor(upperCaseRegion, upperCaseDay); will(returnValue(forecast));
  ...
  public void getFromCacheWhenQueryIsInCache () {
    ...
    context.checking(new Expectations() {{
      exactly(1).of(cache).getFromCache(keys); will(returnValue(forecast));
    }});
